...
extern int limits[];
extern double av_dist_h[],av_dist_p[],m[];
...
int get_3d_lin(CDC * device, int z_min,int z_max, double tolerance){
	...
	Get_epi(epipar, local_p1_h, E1, p_nr_max1);
...
int get_3d(CDC * device, int z_min,int z_max, double tolerance, CProgressCtrl* bar){
	...
	//set the external variables for the statistics
	for(int i=0;i<6;i++)
	...
	//Calculates the essential matrix E1, E2 
	//the rotation matrix R1, R2 and 
	//the translation  vector t1 and t2
	get_rotation_matrix();
	//************Normalize the coordinates******************************************
	if(TABLE_1_LOADED==0)
	...
	//Normalize_coord(local_p1, local_p1_h, in1, ccd1, p_nr_max1); 
	//Normalize_coord(local_p2, local_p2_h, in2, ccd2, p_nr_max2); 
	...
	{
		Plot_epi_no_lim(device, colimage_data2, epipar     , ccd2.height, ccd2.width, p_nr_max1,  0);
		Plot_epi_no_lim(device, colimage_data2, epipar_pal1, ccd2.height, ccd2.width, p_nr_max1,  1);
		Plot_epi_no_lim(device, colimage_data2, epipar_pal2, ccd2.height, ccd2.width, p_nr_max1,  1);
	...
	Solve_3D(global_point, local_p1_h, local_p2_h, matched);
...
int get_rotation_matrix()
{
	...
	//the camera matrix P1
	P1.p11=   R1.a11;
	P1.p12=   R1.a12;
	P1.p13=   R1.a13; 
	P1.p14=  -R1.a11*t1.t1-R1.a12*t1.t2-R1.a13*t1.t3; 
	P1.p21=   R1.a21;
	P1.p22=   R1.a22;
	P1.p23=   R1.a23;
	P1.p24=  -R1.a21*t1.t1-R1.a22*t1.t2-R1.a23*t1.t3; 
	P1.p31=   R1.a31; 
	P1.p32=   R1.a32; 
	P1.p33=   R1.a33; 
	P1.p34=  -R1.a31*t1.t1-R1.a32*t1.t2-R1.a33*t1.t3;
	//the camera matrix P2
	P2.p11=   R2.a11;
	P2.p12=   R2.a12;
	P2.p13=   R2.a13; 
	P2.p14=  -R2.a11*t2.t1-R2.a12*t2.t2-R2.a13*t2.t3; 
	P2.p21=   R2.a21;
	P2.p22=   R2.a22;
	P2.p23=   R2.a23;
	P2.p24=  -R2.a21*t2.t1-R2.a22*t2.t2-R2.a23*t2.t3; 
	P2.p31=   R2.a31; 
	P2.p32=   R2.a32; 
	P2.p33=   R2.a33; 
	P2.p34=  -R2.a31*t2.t1-R2.a32*t2.t2-R2.a33*t2.t3;
	//********************************************************************************************
	...
	*c='\0';
	...
	//********************************************************************************************
	//*********calculate essential matrix E ******************************************************
	R = Neg(  mat_mult    (transpose(R1),   R2));
	t = mat_vec_mult(transpose(R2), (vec_sub(t2,t1)));
	T_x.a11 =  0.0;
	T_x.a12 = -t.t3;
	T_x.a13 =  t.t2;
	T_x.a21 =  t.t3;
...
void Get_cam_transf_matrix(Matrix R, T_vec t)
{
	R = Neg(  mat_mult    (transpose(R2),   R1));
	t = mat_vec_mult(transpose(R2), (vec_sub(t1,t2)));
}
...
void Get_epi(EpiPar epipar[], CamPoint hpoint[], Matrix E, int nr)
{
	int i;
		...
		for(j=0; j <  max2; j++)
		{
			...
			if(distance < tolerance)
			{
				if(s_p > 0 && s_p < n_l)
				{
					cnd_nr++;                    
					match_nr = j; 
					//calculate the residual x2' E x1
					T_vec dvec1;
					dvec1.t1=hpoint1[i].x;
					dvec1.t2=hpoint1[i].y;
					dvec1.t3=hpoint1[i].w;
					dvec1=mat_vec_mult(E1,dvec1);
					residual+=fabs(hpoint2[j].x*dvec1.t1 + hpoint2[j].y*dvec1.t2 + hpoint2[j].w*dvec1.t3);
					res_nr++;
					//quality
...
void Solve_3D(GlobalPoint *global_point, CamPoint *local_p1, CamPoint *local_p2, int point_nr)
{
	int m=4, n=3;           //m: Number of Funktions   n: Number of Variables  n<=m
	double xguess[3];
	double *result;

	for(i_point=0; i_point < point_nr; i_point++)
	{
		xguess[0] =  500.0;
		xguess[1] =  400.0;
		xguess[2] =  -10.0;
...
void Get_3D_point_norm(GlobalPoint *global_point, CamPoint *hp1, CamPoint *hp2, int point_nr)
{
	...
	for(int i=0; i < point_nr; i++)
	{
		...
		global_point[i].X = evec[3].re;
		global_point[i].Y = evec[7].re;
		global_point[i].Z = evec[11].re;
		double W=  evec[15].re;

		global_point[i].X = global_point[i].X /W;
		global_point[i].Y = global_point[i].Y /W;
		global_point[i].Z = global_point[i].Z /W;
	}
}
...
void fcn(int m, int n, double x[], double f[])
{
	...
	//double x_p, y_p;         //Co-ordinates in pixel; (0,0) in left upper corner
	//double x_c, y_c;         //lens distortion compensated
	//double x_b, y_b;
	//double r;
	double x_h,y_h;
...
//	ofstream outdat1,outdat2;
//	outdat1.open("rot_mat.cam1");
//	outdat2.open("rot_mat.cam2");
...
/*	outdat1 << ex1.omega << "  " << ex1.phi << "    " << ex1.kappa << endl;
    outdat1 << endl;
    outdat1 << R1.a11 << "    " << R1.a12 << "    " << R1.a13 << endl;
    outdat1 << R1.a21 << "    " << R1.a22 << "    " << R1.a23 << endl;
    outdat1 << R1.a31 << "    " << R1.a32 << "    " << R1.a33 << endl;

	outdat2 << ex2.omega << "  " << ex2.phi << "    " << ex2.kappa << endl;
    outdat2 << endl;
    outdat2 << R2.a11 << "    " << R2.a12 << "    " << R2.a13 << endl;
    outdat2 << R2.a21 << "    " << R2.a22 << "    " << R2.a23 << endl;
    outdat2 << R2.a31 << "    " << R2.a32 << "    " << R2.a33 << endl;
...
//*******End Read the file with the measured global data***************************
	...
	int limits[11]={0, 150, 300, 450, 600, 750, 900, 1050, 1300, 1450, 1600};
	for(i=0; i< nr_mes; i++)
	{
		for(j=0;j<nr;j++)
		{
			...
			if(delta<5)
			{
				//a translation into the optical centre is needed!
				/*
				r=sqrt(points[i].X*points[i].X + points[i].Y*points[i].Y);
				for(n=0;n<9;n++)
				{
					if(r>=limits[n] && r<limits[n+1])
					{
						diff[n]   += delta;
						x_diff[n] += sqrt(x_delta);
						y_diff[n] += sqrt(y_delta);
						z_diff[n] += sqrt(z_delta);
						s_nr[n]++;
						break;
					}
				}
