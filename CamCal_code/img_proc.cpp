...
int Extract_streaks_roes (CamPoint centr[], uInt8 *binimg, int thres, int min_pix, int max_pix)
/*
   ...
   for (j=0;j<ysize;j++) {
      for (i=0;i<xsize;i++) {
         ...
         if (binimg[ij] > thres) {
            ...
            if (onedge == 0 && pixel > min_pix && pixel <max_pix) {
   ...
   for (j=0;j<ysize;j++) {
      for (i=0;i<xsize;i++) {
         ...
         if (binimg[ij] > thres) {
			...
			n_p=0;
			//nct++;
			calpoint[nct+1].x[n_p]=i;
			calpoint[nct+1].y[n_p]=j;

            onedge= 0;
            if ((i==0) || (i==xsize-1)) onedge= 1;
            if ((j==0) || (j==ysize-1)) onedge= 1;

//            xsum= i; ysum= j; nsum= 1;
			xsum= i*binimg[ij]; ysum= j*binimg[ij]; nsum = binimg[ij];
            ptr= 0; xstack[ptr]= i; ystack[ptr]= j;
            binimg[ij]= 0;
                  ...
                  for (l=j1;l<=j2;l++) {
                     for (k=i1;k<=i2;k++) {
                        ...
                        if (binimg[kl] > thres) {
                           ...
                           found= 1;
						   n_p++;
						   calpoint[nct+1].x[n_p]=k;
						   calpoint[nct+1].y[n_p]=l;
                           xsum += k*binimg[kl]; ysum += l*binimg[kl]; nsum += binimg[kl];
                           ptr++; xstack[ptr]= k; ystack[ptr]= l;
                           binimg[kl]= 0;
                        }
            ...
            if (onedge == 0 && n_p > 5 && n_p < 300 && n_p < MAX_PIXEL) {
			   nct++;
			   centr[nct].x = xsum / (float) nsum; 
			   centr[nct].y = ysum / (float) nsum;
